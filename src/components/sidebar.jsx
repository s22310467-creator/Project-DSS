...
const Sidebar = () => {
  ...
  return (
    <aside className="w-64 min-h-screen sticky top-0 bg-white/30 backdrop-blur-xl border-r border-white/40 flex flex-col shadow-2xl">
        ...
        {menuItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              ${isActive 
                ? 'bg-emerald-500/20 text-emerald-800 border border-emerald-500/20 shadow-sm' 
                : 'text-gray-600 hover:bg-emerald-50/50 hover:text-emerald-700'}
            `}
          >
            {({ isActive }) => (
              <>
                <item.icon className={`w-5 h-5 transition-colors ${isActive ? 'text-emerald-600' : 'text-gray-400 group-hover:text-emerald-500'}`} />
      ...
      <div className="p-6">
        <div className="p-4 bg-emerald-600/10 rounded-2xl border border-emerald-200/50">
          <p className="text-xs text-emerald-800 font-semibold mb-1">Status Sistem:</p>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
            <p className="text-[10px] text-emerald-700 uppercase tracking-wider font-bold">Ready to Calculate</p>
          </div>
        </div>
      </div>
    </aside>
