...
const Dashboard = () => {
  ...
  const summaryStats = [
    { id: 1, name: 'Total Kriteria', value: '5', icon: Target, color: 'text-teal-600' },
    { id: 2, name: 'Total Supplier', value: '12', icon: Users, color: 'text-emerald-600' },
    { id: 3, name: 'Status Konsistensi AHP', value: 'Konsisten', icon: TrendingUp, color: 'text-green-600' },
  ];
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {/* Tombol Aksi Cepat dengan efek Glass */}
      ...
      {/* Grid Statistik Utama (Poin 1: Ringkasan cepat) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {summaryStats.map((item) => {
          const Icon = item.icon;
          return (
            <div key={item.id} className="glass-card p-6 flex items-start gap-5 transition-transform hover:scale-[1.02]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Bagian Visualisasi (Contoh Poin 9 - Ranking Visual) */}
        ...
        {/* Bagian 3 Besar (Poin 9 - Ranking Tabel) */}
            ...
            {topSuppliers.map((sup) => (
              <div key={sup.rank} className="flex items-center gap-4 p-4 bg-white/70 rounded-xl border border-white/50 shadow-sm transition-all hover:bg-white">
                ...
                <div className='flex-1'>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Skor Akhir: <span className='font-mono font-bold text-teal-700'>{sup.score}</span></p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Bagian Alur Proses (User Guide Cepat) */}
